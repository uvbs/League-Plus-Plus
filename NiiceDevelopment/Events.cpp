...
void Events::OnRender()
{
	...
	if (GPlugin->GetMenuOption("Mouse.Lines")->Enabled())
	{
	...
	if (GPlugin->GetMenuOption("Mouse.Informations")->Enabled())
	{
	...
	if (GPlugin->GetMenuOption("GUI")->Enabled())
	{
		...
		GRender->DrawTextW(Vec2(GPlugin->GetMenuOption("Position.X")->GetInteger() + 150, GPlugin->GetMenuOption("Position.Y")->GetInteger() + 120), Vec4(255, 255, 255, 255), "%s (%d)", FocusedUnit->GetType() == FL_MISSILE ? "Missile" : FocusedUnit->GetType() == FL_TURRET ? "Turret" : FocusedUnit->GetType() == FL_HERO ? "Hero" : FocusedUnit->GetType() == FL_CREEP ? "Creep" : "invalid", FocusedUnit->GetType());
		GRender->DrawTextW(Vec2(GPlugin->GetMenuOption("Position.X")->GetInteger() + 20, GPlugin->GetMenuOption("Position.Y")->GetInteger() + 140), Vec4(160, 160, 160, 255), "Position (X | Y | Z):");
		GRender->DrawTextW(Vec2(GPlugin->GetMenuOption("Position.X")->GetInteger() + 150, GPlugin->GetMenuOption("Position.Y")->GetInteger() + 140), Vec4(255, 255, 255, 255), "%d | %d | %d", static_cast<int>(FocusedUnit->GetPosition().x), static_cast<int>(FocusedUnit->GetPosition().y), static_cast<int>(FocusedUnit->GetPosition().z));
